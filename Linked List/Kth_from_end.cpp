...
public:
  ...
  int kthFromEnd(int k) 
  {
   ...
   //1st we are keeping both the nodes at head
   node* slow = head;
   node* fast = head;
   
   for(int idx = 0; idx < k; idx++)
   {
       fast = fast->next; //fast hamesha slow se k steps aage hai
   }
   
   while(fast->next != NULL) //jab tak fast null pe reach karega tab tak slow us particular position pe present hoga
   {
       slow = slow->next;
       fast = fast->next;
   }
   
   return slow->val; //return the data
